...
class GelloGui:
    ...
    def setup_step_2(self):
        ...
        if os.path.exists(self.urdf_var.get()):
            ttk.Label(self.container, text="✅ URDF Found", foreground="#4caf50").pack()
        else:
            ttk.Label(self.container, text="❌ URDF Not Found!", foreground="#f44336").pack()
    ...
    def setup_step_3(self):
        ...
        instr = (
            ...
            "• Joint 2 (Shoulder): -90° (POINTING UP)\n"
            "• Joint 3 (Elbow): +90° (BENT FORWARD)\n"
            "• Joint 4 (Wrist1): -90° (POINTING DOWN)\n"
            "• Joint 5 (Wrist2): -90°\n"
            "• Joint 6 (Wrist3): 0°\n\n"
            "The arm must be held steady in this pose for initialization."
        )
    ...
    def initialize_arm(self):
        ...
        config["urdf_path"] = self.urdf_var.get()
    ...
    def toggle_run(self):
        if not self.is_running:
            ...
            self.start_btn.configure(text="STOP", background="#f44336")
    ...
    def update_status(self):
        if self.comp and hasattr(self.comp, 'tau_g'):
            try:
                arm_pos, _ = self.comp.get_joint_states()
